...
namespace Engine {
		Sprite::Sprite()
		{
			//Always initialize buffer IDs to 0
			_vboID = 0;
		}
		...
		Sprite::~Sprite()
		{
			//Always remember to delete buffers when done!
			if (_vboID != 0) {
		...
		void Sprite::init(float x, float y, float width, float height, std::string texturePath) {
			...
			//Need 6 vertices, and each vertex has 2
			//floats for X and Y
			Vertex vertexData[6];
		...
		void Sprite::draw() {
			...
			//Tell opengl to use the first
			//attribute array. Only need one array right
